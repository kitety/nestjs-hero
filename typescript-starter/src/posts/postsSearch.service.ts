...
export default class PostsSearchService {
  ...
  async search(query: string, offset?: number, limit?: number, startId = 0) {
    let separateCount = 0;
    if (startId) {
      separateCount = await this.count(query, ['title', 'paragraphs']);
    }
    ...
    return { results, count: startId ? separateCount : count };
  }
